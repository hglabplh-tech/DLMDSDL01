...
from RAGUtils import get_embedding, get_db_temp_path, build_vectors
...
def semantic_asag(student_ans, ref_ans):
    query = student_ans
    ...
    vector_db =  build_vectors(docs, db_path, True)

    scored_result = vector_db.similarity_search_with_score(query, k=1)
    relevance_result = vector_db.similarity_search_with_relevance_scores(query, k=1)
...
def get_jaccard_sim(ans, ref):
    ...
    if score_union:
        score = len(score_intersect) / len(score_union)
    else:
        score = 0.0
    ...
    if ref_set:
        relevance = len(rel_intersect) / len(ref_set)
    else:
        relevance = 0.0
...
if __name__ == '__main__':
    ...
    student = "Mitochondria are the powerhouse for cells."
    print(f"Similarity Score: {get_asag_score(student, ref)}")
   ...
   # ans = "Plants use sunlight to make food energy."
    ...
    score, relevance = semantic_asag(ans, ref)
    ...
    print(f"Keyword Score: {rule_based_grading(student, key_terms)}")

    ref = "The mitochondria is the powerhouse of the cell"
    ans = "Mitochondria is a cell powerhouse"
    #ans = "I saw a pink elefant"
    score, relevance = get_jaccard_sim(ans, ref)
